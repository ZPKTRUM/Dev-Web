...
document.addEventListener("DOMContentLoaded", () => {
  ...
  formulario.addEventListener("submit", function(e) {
    ...
    const nombreCliente = document.getElementById("nombreCliente") ? document.getElementById("nombreCliente").value.trim() : "";
    ...
    const gravedadDanio = document.getElementById("gravedadDanio").value;
    ...
    if (!/^\+56 9 \d{4} \d{4}$/.test(telefono)) {
      alert("Teléfono inválido. Ejemplo: +56 9 1234 5678");
      return;
    }

    // Validar patente chilena simple (ej: AB1234 o ABCD12)
    if (!/^[A-Z]{2,4}\d{2,4}$/.test(patente)) {
      alert("Patente inválida. Ejemplo: ABCD12 o AB1234");
      return;
    }

    // Evitar duplicados RUT + póliza
    if (siniestroManager.buscarSiniestro(rut, numeroPoliza)) {
      alert("Ya existe un siniestro registrado para este RUT y póliza.");
      return;
    }

    if (!validarNombreCliente(nombreCliente)) {
      alert("Nombre inválido. Solo letras y mínimo 2 caracteres.");
      return;
    }
    ...
    const datos = {
      ...
      tipoSeguroDetalle: gravedadDanio,
...
// Validar nombre: solo letras, espacios y mínimo 2 caracteres
